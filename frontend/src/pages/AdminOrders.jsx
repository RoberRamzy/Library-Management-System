...
export default function AdminOrders() {
  ...
  const handleConfirmOrder = async (orderID) => {
    if (!window.confirm('Are you sure you want to confirm this order? Stock will be automatically added.')) return
    setLoading(true)
    try {
      const res = await fetch(`${apiBase}/admin/confirm-order/${orderID}`, { method: 'PUT' })
  ...
  const handleBookChange = (isbn) => {
    ...
    setOrderForm(prev => ({ ...prev, ISBN: isbn, PubID: book ? book.PubID.toString() : prev.PubID }))
  }
  ...
  return (
    <div className="page">
        ...
        <div className="admin-section" style={{ marginBottom: '2rem' }}>
          ...
          <form onSubmit={handleCreateOrder} className="form-grid">
            <div className="form-group">
              ...
              <select className="input" value={orderForm.ISBN} onChange={(e) => handleBookChange(e.target.value)} required>
                <option value="">Select Book</option>
                {books.map(book => (
                  <option key={book.ISBN} value={book.ISBN}>{book.ISBN} - {book.Title}</option>
                ))}
              </select>
            </div>
            <div className="form-group">
              <label>Quantity *</label>
              <input className="input" type="number" value={orderForm.Quantity} onChange={(e) => setOrderForm({ ...orderForm, Quantity: parseInt(e.target.value) })} required />
            </div>
            <div style={{ gridColumn: '1 / -1', marginTop: '1rem' }}>
              <button className="button button-primary" type="submit" disabled={loading}>Place Order</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="admin-section">
        ...
        {/* This wrapper ensures the table doesn't overflow the white card */}
        <div style={{ width: '100%', overflowX: 'auto', borderRadius: '8px' }}>
          <table className="users-table" style={{ minWidth: '1000px', width: '100%' }}>
            <thead>
              <tr>
                <th>Order ID</th>
                <th>Date</th>
                ...
                <tr key={order.orderID}>
                  ...
                  <td>
                    {order.status === 'Pending' ? (
                      <button className="button button-primary button-small" onClick={() => handleConfirmOrder(order.orderID)}>
                        Confirm Order
                      </button>
